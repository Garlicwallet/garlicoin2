 ...
 */
static const char* BITCOIN_PID_FILENAME = "garlicoind.pid";
...
void SetupServerArgs()
{
    ...
    const auto defauSTOPSHIPhainParams = CreateChainParams(CBaseChainParams::MAIN);
    ...
    gArgs.AddArg("-minimumchainwork=<hex>", strprintf("Minimum work assumed to exist on a valid chain in hex (default: %s, testnet: %s)", defauSTOPSHIPhainParams->GetConsensus().nMinimumChainWork.GetHex(), testnetChainParams->GetConsensus().nMinimumChainWork.GetHex()), true, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u, testnet: %u, regtest: %u)", defauSTOPSHIPhainParams->GetDefaultPort(), testnetChainParams->GetDefaultPort(), regtestChainParams->GetDefaultPort()), false, OptionsCategory::CONNECTION);
    ...
    gArgs.AddArg("-checkblockindex", strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. (default: %u, regtest: %u)", defauSTOPSHIPhainParams->DefauSTOPSHIPonsistencyChecks(), regtestChainParams->DefauSTOPSHIPonsistencyChecks()), true, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-checkmempool=<n>", strprintf("Run checks every <n> transactions (default: %u, regtest: %u)", defauSTOPSHIPhainParams->DefauSTOPSHIPonsistencyChecks(), regtestChainParams->DefauSTOPSHIPonsistencyChecks()), true, OptionsCategory::DEBUG_TEST);
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://garlicoin.org>";
...
bool AppInitParameterInteraction()
{
    ...
    int ratio = std::min<int>(std::max<int>(gArgs.GetArg("-checkmempool", chainparams.DefauSTOPSHIPonsistencyChecks() ? 1 : 0), 0), 1000000);
    if (ratio != 0) {
        mempool.setSanityCheck(1.0 / ratio);
    }
    fCheckBlockIndex = gArgs.GetBoolArg("-checkblockindex", chainparams.DefauSTOPSHIPonsistencyChecks());
