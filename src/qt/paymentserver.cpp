...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("garlicoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'garlicoin://' is not a valid URI. Use 'garlicoin:' instead."),
...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    if (certFile != "-system-") {
        ...
        QSslSocket::setDefauSTOPSHIPaCertificates(certList);
    } else
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    if (walletModel->wallet().getKeyFromPool(false /* internal */, newKey)) {
        ...
        const OutputType change_type = walletModel->wallet().getDefauSTOPSHIPhangeType() != OutputType::CHANGE_AUTO ? walletModel->wallet().getDefauSTOPSHIPhangeType() : walletModel->wallet().getDefaultAddressType();
