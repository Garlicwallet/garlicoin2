...
- [Development guidelines](#development-guidelines)
    - [General Garlicoin Core](#general-garlicoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Garlicoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Garlicoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/GarlicoinOrg/Garlicoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_garlicoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_garlicoin --log_level=test_suite
$ valgrind -v --leak-check=full src/garlicoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_garlicoin.coverage/index.html`.
```
...
To profile a running garlicoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep garlicoind` -- sleep 60
```
...
Garlicoin Core can be compiled with various "sanitizers" enabled, which add
...
file descriptors it will cause problems with Garlicoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Garlicoin's
