...
class FilelockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second garlicoind instance using the same datadir")
        expected_msg = "Error: Cannot obtain a lock on data directory {}. Garlicoin Core is probably already running.".format(datadir)
        self.nodes[1].assert_start_raises_init_error(extra_args=['-datadir={}'.format(self.nodes[0].datadir), '-noserver'], expected_msg=expected_msg)

        if self.is_wallet_compiled():
            wallet_dir = os.path.join(datadir, 'wallets')
            self.log.info("Check that we can't start a second garlicoind instance using the same wallet")
